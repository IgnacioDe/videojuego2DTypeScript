...
export class MovePhysics extends Container {
    ...
    public update(deltaSeconds: number) {
        this.x += this.speed.x * deltaSeconds + 0.5 * this.acceleration.x * Math.pow(deltaSeconds, 2);
        this.y += this.speed.y * deltaSeconds + 0.5 * this.acceleration.y * Math.pow(deltaSeconds, 2);
